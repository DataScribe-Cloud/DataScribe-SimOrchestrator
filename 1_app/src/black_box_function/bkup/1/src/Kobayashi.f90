...
MODULE Heat_eqn
    ...
    real*8, parameter, private :: gamma = alpha*dt/DX**2;    !gamma;      value of gamma to measure time-step size
...
program Pure_dendrite
    ...
    OPEN(9, FILE='results/TIME_THICKNESS_1.dat')
    OPEN(13,FILE='microstructures/con_PHIMAX.dat')
    OPEN(15,FILE='microstructures/initial.dat')
...
210         CONTINUE
        ...
        subroutine printdata()
            ...
            ELSEIF(itimes.EQ.0 .OR. MOD(itimes,wrt_cycle).EQ.0)THEN
                print*, 'iter',itimes,'time',time,'PHITOT',phitot
                print*, 'new wrt_cycle...'
            ENDIF
666         FORMAT(1X,I9,10(2X,E14.7))
            ...
            IF(itimes.EQ.0 .OR. MOD(itimes,file_cycle).EQ.0)THEN
                ...
                open(unit=13, file='results/Temperature'//ch//'.plt')
        ...
        subroutine pfm_evolution()
            ...
            DO I=0, IG, 1
                DO J=0, JG, 1
                    ...
                    mm(I,J)= (alpha/pi)*atan(gamma*(T_e-temp(I,J)))
                    !mm2(I,J) = -(alpha/3.14)*ATAN(gamma*( C1(I,J) - T_e ) )*(1.0-PHI1(1,I,J))
